...
SafeBox 是 SBOX v1 的纯 Flutter + Dart 跨平台客户端。它把任意本地文件或 UTF-8 文本加密成可公开托管的 `.sbox` 密文；公开 GitHub/Gitee 仓库、HTTPS 站点、网盘或普通文件夹只会看到随机对象名、密文分片和加密后的 `catalog.sbox`。只有持有匹配 12 词 BIP39 助记词的人才能临时重建私钥并解密。
...
- 身份：BIP39 12 词经固定 profile 确定性派生 RSA-3072 与 Catalog Ed25519 签名身份。
...
- 目录：`catalog.sbox` 自身加密，内层规范 JSON 使用 Ed25519 签名；客户端执行回滚、分叉、重复键和签名检查。
- 持久化：公钥、Key ID、公开数据源配置和完整 `.sbox` 原件可以永久保存。助记词、BIP39 Seed、RSA/Ed25519 私钥、DEK 和解密后的 Catalog 不进入普通存储、平台安全存储、上传队列或日志。
...
- GitHub、Gitee 和只读 HTTPS 数据源；匿名读取、令牌写入、流式 Base64 上传、对象先行、Catalog 条件提交。
...
- 已验证条目支持编辑标题/说明/标签和墓碑逻辑删除；每次增加条目 revision 与 Catalog generation、重新签名并重新加密，且不会自动删除永久 SBOX 对象。
