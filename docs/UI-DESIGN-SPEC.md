...
1. **云端文件**：统一承载文件查看、上传、下载和解密四个动作。
2. **设置**：统一承载安全身份、云端备份和使用习惯。

原有的“上传”“下载”“解密”“双云设置”“身份”不再作为独立导航入口。这样用户只需要理解两个问题：
...
- 每个文件分别提供“下载”和“解密”动作；下载只保存加密备份，不要求输入私钥，解密时再要求恢复词。
...
1. 用户在文件行点击“下载”；下载只保存加密备份，不要求输入私钥。
2. 按钮进入“准备中”状态，避免重复点击。
3. 下载完成后，文件行显示“解密”；用户点击后输入恢复词并完成校验。
4. 解密完成后，用户可点击“打开文件”或“打开文件夹”。
5. 明文临时文件的清理策略由“设置 → 使用习惯”控制。
